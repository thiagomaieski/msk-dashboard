        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '20px 24px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              ...
              {cliente.cpfCnpj && <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>{formatCpfCnpj(cliente.cpfCnpj)}</div>}
            </div>
          </div>
...
export default function ClientesPage() {
  ...
  const lista = useMemo(() => {
    let l = [...(data.clientes || [])];
    if (search) l = l.filter(c => (c.nome || '').toLowerCase().includes(search.toLowerCase()) || (c.segmento || '').toLowerCase().includes(search.toLowerCase()) || (c.email || '').toLowerCase().includes(search.toLowerCase()));
    return sortData(l, sort);
  }, [data.clientes, search, sort]);

  const getStatusCliente = (c) => {
    ...
    if (temProjeto || temRec) return 'ativo';
    return 'inativo';
  };
    ...
    <div>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: 'var(--text3)' }}>{lista.length} cliente(s)</span>
            ...
            return (
              ...
              >
                ...
                <div style={{ position: 'absolute', top: 14, right: 14, display: 'flex', gap: 6, alignItems: 'center' }}>
                  <span style={{ width: 7, height: 7, borderRadius: '50%', background: status === 'ativo' ? 'var(--green)' : 'var(--text3)', display: 'inline-block' }} />
                  <span style={{ fontSize: 10, color: status === 'ativo' ? 'var(--green)' : 'var(--text3)', fontWeight: 600 }}>{status === 'ativo' ? 'ATIVO' : 'INATIVO'}</span>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5, marginBottom: 14 }}>
                  ...
                  {c.cpfCnpj && (
                    <div style={{ fontSize: 12, color: 'var(--text3)', fontFamily: 'var(--mono)' }}>{formatCpfCnpj(c.cpfCnpj)}</div>
                  )}
                </div>
